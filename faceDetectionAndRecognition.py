...
# # Function to play the audio on Windows
def play_audio(audio_file):
    # subprocess.Popen(['start', '', audio_file], shell=True)
    try:
        # Run the audio player in the background with detached streams
        subprocess.Popen(['start', '', audio_file], shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                         stdin=subprocess.PIPE)
    except Exception as e:
        print(f"Error while playing audio: {e}")

# # Example usage
# audio_file = 'emergency_alarm.mp3'
